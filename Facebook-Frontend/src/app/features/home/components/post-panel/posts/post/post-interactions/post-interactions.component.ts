...
export class PostInteractionsComponent {
  ...
  getTitleByName(name: string): string {
    const interaction = this.interactionIcons.find(
      (item) => item.name === name
    );
    ...
    if (!this.interactionDetails()) {
      this.interactionService.getInteractionPost(this.postId()).subscribe({
        next: (response: ApiResponse) => {
          ...
          const allNames = names.map(
            (item: { interactor_name: string }) => item.interactor_name
          );
          let finalString = '';
          if (totalInteractions > maxDisplay) {
            finalString = allNames
              .concat(`và ${totalInteractions - maxDisplay} người khác`)
              .join('<br>');
          } else {
    ...
    this.interactionService
      ...
      .subscribe({
        next: (response: ApiResponse) => {
          ...
          const finalString =
            `<h1 class="font-bold">${title}</h1>` + allNames.join('<br>');
